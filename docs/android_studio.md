...
Na tela de configuração do AVD Manager clique em `+ Create Virtual Device` e clique em `Next`. Selecione o dispositivo que deseja executar e clique em `Next`, depois selecione uma imagem do sistema que seja _API 23 +_, depois clique em `Next` e `Finish`.
