...
rem Assume que o script está em .../virtual-master/bin/ e o diretório de trabalho é virtual-master
cd %~dp0..

SET HEAP_OPTS=-Xms64m -Xmx512m -XX:PermSize=32m -XX:MaxPermSize=128m

SET JMX_OPTS=-Dcom.sun.management.jmxremote.port=9988 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false
...
SET CONFIG_PATH="%cd%/config/config-master.properties"

FOR /r %%i IN (virtual-master-*.jar) DO (
    SET "APP_JAR=%%i"
)

SET MAIN_CLASS="br.com.master.Application"
