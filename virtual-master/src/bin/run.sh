...
export JMX_OPTS="-Dcom.sun.management.jmxremote.port=9988 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false"
...
export DEBUG_OPTS="-XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=logs -XX:ErrorFile=logs/server_java_error$(date +'%Y-%m-%d-%H%M%S').log"

export JAVA_OPTS="-server -Dfile.encoding=UTF-8 $GC_OPTS $HEAP_OPTS $JMX_OPTS $DEBUG_OPTS"

export CONFIG_PATH="$(pwd)/config/config-master.properties"

export APP_JAR=""

for i in virtual-master-*.jar
do
    APP_JAR="$APP_JAR:$i";
done

export MAIN_CLASS="br.com.master.Application"
