...
public class MasterTaskBusiness extends DBusiness<BlocoTO> implements IMasterTask {
    ...
    public void instance() throws MasterException {
        balance.atualizarInstanceId(info -> {
            try (Response response = httpGet(info.getHomePageUrl())) {
                final Slave slave = mapper.readValue(response.body().string(), Slave.class);
    ...
    public void gravacao() throws MasterException {
        final Iterator<BlocoTO> blocos = programmaticTransaction(() -> {
            return business.carregarParaGravacao().iterator();
        });

        while (blocos.hasNext()) {
            final BlocoTO bloco = blocos.next();
            ...
            programmaticTransaction(() -> {
                try (Response response = httpPost(new FileInputStream(bloco.getDiretorioOffLine()),
                        balance.volumeUrlSlave(bloco.getInstanceId()),
    ...
    public void replicacao() throws MasterException {
        ...
        while (blocos.hasNext()) {
            final BlocoTO bloco = blocos.next();
            final String instanceIdOrigem = bloco.getInstanceId();
            balance.percorrerInstanceId(instanceId -> {
                if (!instanceIdOrigem.equals(instanceId)) {
                    final boolean exists = programmaticTransaction(() -> {
                    ...
                    if (!exists) {
                        ...
                        return programmaticTransaction(() -> {
                            try (Response response = httpPost(balance.volumeUrlSlave(instanceIdOrigem), "replicacao",
                                    bloco.getUuid(), params)) {
                                if (Status.NO_CONTENT.getStatusCode() != response.code()) {
                                    throw new MasterException(response.body().string()).status(Status.BAD_REQUEST);
                                }
                            } catch (final Exception e) {
    ...
    public void limpar() throws MasterException {
        final File directory = new File(balance.getPathTmpDirectory());
        final File[] files = directory.listFiles(file -> !file.getName().toLowerCase().endsWith(VIRTUAL_EXTENSION));
    ...
    public void exclusao() throws MasterException {
        ...
        while (blocos.hasNext()) {
            final BlocoTO bloco = blocos.next();
            programmaticTransaction(() -> {
                try (Response response = httpDelete(balance.volumeUrlSlave(bloco.getInstanceId()), "exclusao",
                        bloco.getUuid())) {
                    if (Status.NO_CONTENT.getStatusCode() != response.code()) {
                        throw new MasterException(response.body().string()).status(Status.BAD_REQUEST);
                    }
