...
public class ConfiguracaoResource {
    ...
    @ApiOperation(
            value = "Consulta os metadados da configuração",
            nickname = "consulta",
            notes = "<p>A consulta é usada para recuperar os <strong>metadados</strong> da <strong>configuração</strong>. Se a <strong>configuração</strong> existir os <strong>metadados</strong> serão enviados no corpo da mensagem de resposta no formato <strong>json</strong> caso contrário, o status <strong>404</strong> indicando que a configuração não existe ou não foi localizado.</p>",
    ...
    @ApiOperation(
            value = "Altera os metadados da configuração",
            nickname = "alteracao",
            notes = "<p>A alteração é usada para manter os <strong>metadados</strong> da única configuração cadastrada. Se a operação for realizada com sucesso, o status <strong>204</strong> será retornado caso contrário, a mensagem de erro.</p>",
            produces = MediaType.APPLICATION_JSON_VALUE
            )
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> alteracao(
        ...
        JsonNode domain = mapper.valueToTree(configuracao);
        try {
